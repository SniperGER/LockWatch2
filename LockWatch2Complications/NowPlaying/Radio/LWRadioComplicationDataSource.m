//
// LWRadioComplicationDataSource.m
// LockWatch
//
...
#import "LWRadioComplicationDataSource.h"

extern NSString* CLKStringForComplicationFamily(long long family);

@implementation LWRadioComplicationDataSource

- (instancetype)initWithComplication:(NTKComplication*)complication family:(long long)family forDevice:(CLKDevice*)device {
	if (self = [super initWithComplication:complication family:family forDevice:device]) {
		_queue = dispatch_queue_create("com.apple.NanoTimeKit.NTKRadioComplicationDataSource", dispatch_queue_attr_make_with_qos_class(DISPATCH_QUEUE_SERIAL, QOS_CLASS_DEFAULT, 0));
...
- (CLKComplicationTimelineEntry*)_defaultTimelineEntry {
	...
	CLKComplicationTimelineEntry* defaultTimelineEntry = [CLKComplicationTimelineEntry entryWithDate:timelineEntry.entryDate complicationTemplate:[timelineEntry radioTemplateForComplicationFamily:self.family]];
...
- (void)_updateWithOrigin:(id)origin {
	dispatch_async(_queue, ^{
		...
		[defaultTimelineEntry setComplicationTemplate:[timelineEntry radioTemplateForComplicationFamily:self.family]];
